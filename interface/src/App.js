...
class App extends Component {
  ...
  componentDidMount() {
    window.urbit = new Urbit("http://localhost", "lidlut-tabwed-pillex-ridrup");
  ...
  init = () => {
    this.getEntries().then(
      (result) => {
        console.log("get entries results =>", result);
  ...
  submitEdit = async (id, txt) => {
    if (txt !== null) {
      try {
        const result = await window.urbit.poke({
          app: "cue",
          mark: "cue-action",
        ...
        console.log("submitEdit error => ", e);
      }
    } else this.cancelEdit(id);
  };

  submitNew = async (id, txt) => {
    try {
      const result = await window.urbit.poke({
        app: "cue",
        mark: "cue-action",
      ...
      console.log("submitNew result => ", result);
    } catch (e) {
      console.log("submitNew error => ", e);
    }
  };
